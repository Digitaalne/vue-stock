  ...
  <div id="position">
    <md-table md-card>
      ...
      <md-table-row v-for="position in positionList" :key="position.asset_id">
        ...
        <md-table-cell>{{position.avg_entry_price.toFixed(3)}}</md-table-cell>
        ...
        <md-table-cell>{{(position.unrealized_plpc * 100).toFixed(2)}}</md-table-cell>
        <md-table-cell>{{position.unrealized_intraday_pl}}</md-table-cell>
        <md-table-cell>{{(position.unrealized_intraday_plpc * 100).toFixed(2)}}</md-table-cell>
        <md-table-cell>{{position.current_price}}</md-table-cell>
        <md-table-cell>{{position.lastday_price}}</md-table-cell>
        <md-table-cell>{{(position.change_today* 100).toFixed(2)}}</md-table-cell>
      </md-table-row>
    </md-table>
  </div>
</template>
