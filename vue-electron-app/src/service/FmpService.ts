...
function buildHistoricalDataUrl(startDate:Date, endDate: Date, symbol:string, timeframe:string){
    let startDateString = startDate.toISOString().split('T')[0]
    let endDateString = endDate.toISOString().split('T')[0]
    if(timeframe !== "1day"){
        return fmpHistoricalUrl+timeframe+"/"+symbol+"?"+"from="+startDateString+"&to="+endDateString+"&apikey="+getApiKey()
    } else {
        return fmpHistoricalDayUrl+symbol+"?"+"from="+startDateString+"&to="+endDateString+"&apikey="+getApiKey()
    }
}
export default {
    async getHistoricalData(startDate:Date, endDate: Date, symbol:string, timeframe:string){
        timeframe = convertTimeframe(timeframe)
        let url = buildHistoricalDataUrl(startDate,endDate, symbol, timeframe)
        let dataObject = await axiosService.get(url)
        dataObject = dataObject.reverse()
        let response = []
        let volumeResponse = []
        for(let i = 0; i<dataObject.length; i++){
            let time = new Date(dataObject[i].date).getTime()
            response.push([time, dataObject[i].open, dataObject[i].high, dataObject[i].low, dataObject[i].close]) 
            volumeResponse.push([time, dataObject[i].volume])
        }
        return {stock_data_list:response, stock_volume_list:volumeResponse, name:symbol}
    },
    searchStockSymbol(stock:String){
        let url = fmpTickerSearchUrl + "?query="+stock+"&limit=10&apikey="+getApiKey()
        return axiosService.get(url)
    },
    getStockNews(stock:string){
        let url = fmpStockNewsUrl+"?tickers="+stock+"&limit=10&apikey="+getApiKey()
        return axiosService.get(url)
    }
}